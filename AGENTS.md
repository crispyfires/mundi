...
- **Sound Effects**: `SoundPlayer` class wraps two `GtkMediaFile` instances (correct/wrong), reads `sound-effects` GSettings key before playing. Sound files are from the freedesktop sound theme at `{DATADIR}/sounds/freedesktop/stereo/`
